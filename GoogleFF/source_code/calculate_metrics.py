...
def load_model_runs_for_experiment(
  ...
  """
  experiment_run_path = os.path.join(model_run_base_path, experiment)
  gauge_xarrays = []
  for gauge in gauges:
    gauge_run_file = os.path.join(experiment_run_path, f'{gauge}.nc')
    if os.path.exists(gauge_run_file):
      with open(gauge_run_file, 'rb') as f:
        model_run = xr.load_dataset(f)
...
def load_model_runs_for_experiment_decommissioned(
    experiment: str,
    gauges: Sequence[str],
    gauge_to_model_path: Mapping[str, str],
) -> xr.Dataset:
