...
config <- yaml::yaml.load_file("configuracion_caudales.yml")
...
readr::write_csv(x = caudales_agregados, path = paste0("work/caudales_agregados.csv"))
# ------------------------------------------------------------------------------

# test <- caudales_agregados %>% 
#   filter(estacion == 133, ancho_ventana == 18, pentada_fin == 42)
# 
# ajuste <- logspline::logspline(x = test$caudal_agregado,lbound = 0)
# plot(ajuste)
# 
# percentil <- logspline::plogspline(q = 15000,fit = ajuste)
# 
# sdi <- qnorm(p = percentil)
# 
# x <- seq(0,40000,100)
# y <- logspline::plogspline(q = x,fit = ajuste)
# 
# plot(x,y)
