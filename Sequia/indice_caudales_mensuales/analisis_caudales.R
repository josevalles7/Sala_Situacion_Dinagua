...
codcuenca = "SL"
filename = paste0("input/",codcuenca,"_monthly_santalucia.csv")
data <- read.csv(filename,header = TRUE)
data$Fecha <- as.Date(data$Fecha,format = "%d/%m/%Y")
station_name <- tools::file_path_sans_ext(basename(filename))

caudales_mensuales <- data
...
estacion.seleccion <- caudales_mensuales

estacion.seleccion$Ano = year(estacion.seleccion$Fecha)
estacion.seleccion$Month = month(estacion.seleccion$Fecha)
