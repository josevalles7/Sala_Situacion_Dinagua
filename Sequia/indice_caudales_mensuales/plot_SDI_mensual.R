...
name.station <- "durazno"
name.station_plot <- "Durazno"
codigo.station <- "2206"
scale.sdi <- 6

filename <- paste0("output/",scale.sdi,"-month","_CompleteSDI_junio_",gsub(" ","",name.station),".txt")
...
p <- ggplot(d,aes(x=Fecha,y=GammaSDI)) + 
  ...
  scale_x_date(date_breaks = "1 month", 
               ...
               limits = c(as.Date("2021-05-01"),as.Date("2023-07-01"))) + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 20),
                     limits = c(-3,3)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(title = "Indice Sequía en Caudales (SDI)",
       subtitle = paste0("Estación:", name.station_plot, " cod: ", codigo.station, " - Escala temporal de ", scale.sdi, " Meses"),
       x = "Fecha",
       y = "SDI (-)", 
...
dev.print(png,filename='output/durazno_SDI_jun2023_6M.png',
    width = 1250 , height = 593, res=100)
